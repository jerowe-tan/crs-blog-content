import { getBlogs } from "@model/blog";
...
export const GET:APIRoute = async ({cookies, url, params})=>{
  ...
  const blog = await getBlogs(page, limit);
  const rowData = blog;

  for(let i = 0; i < rowData.data.length; i++){
    const thumbnailFile = `${url.protocol}//${url.hostname}/api/blog/thumbnail/${rowData.data[i].slug}`;
    const blogFile = `${url.protocol}//${url.hostname}/api/blog/${rowData.data[i].slug}`;
